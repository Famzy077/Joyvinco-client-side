...
// --- LOGIN - User login (No changes in this function) ---
const login = async (req, res) => {
  try {
    const { email, password } = req.body;
    if (!email || !password) {
...
// --- READ - Get all users (No changes in this function) ---
const getAllUsers = async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 10;
...
// --- READ - Get user by ID (No changes in this function) ---
const getUser = async (req, res) => {
  try {
    const { id } = req.params;
    const user = await prisma.user.findUnique({
...
// --- UPDATE - Update user account (No changes in this function) ---
const updateAccount = async (req, res) => {
  try {
    const { id } = req.params;
    const { email, password, newPassword } = req.body;
...
// --- DELETE - Delete user account (No changes in this function) ---
const deleteAccount = async (req, res) => {
  try {
    const { id } = req.params;
    const { password } = req.body;
...
// --- checkEmail (No changes in this function) ---
const checkEmail = async (req, res) => {
  const { email } = req.query;
  if (!email) {
    return res.status(400).json({ success: false, message: 'Email is required' });
...
const forgotPassword = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      // --- CHANGED: Use resend.emails.send ---
      await resend.emails.send({
        from: FROM_EMAIL,
        to: user.email,
        subject: 'Your Password Reset Request',
...
// --- resetPassword (No changes in this function) ---
const resetPassword = async (req, res) => {
  try {
    const { token, newPassword } = req.body;
    if (!token || !newPassword) {
