...
export const HeaderPage = () => {
  ...
  return (
    <div className='sticky top-0 z-50'>
      <header className='flex items-center justify-between p-2 bg-green-100 shadow-md max-md:hidden'>
        <Link href={'/home'}>
        ...
        <div onClick={toggleSearchModal} className='border hover:border-green-600 border-gray-500 w-[12rem] flex items-center p-1 rounded-[5px] m-auto text-[8px] cursor-pointer'>
          ...
          <i className='ml-2 text-sm text-zinc-600'>Search products..</i>
        </div>
        <Link href="/cart">
      ...
      {showSearch && (
        <div className="fixed inset-0 bg-transparent bg-opacity-50 h-screen flex justify-center items-start pt-[4.6rem] z-[90]">
          <div ref={searchModalRef} className="bg-white z-[100] w-[50%] max-h-[80vh] rounded-lg p-4 shadow-lg relative flex flex-col">
            <button onClick={toggleSearchModal} className="absolute text-gray-500 transition-all right-3 top-2 hover:text-red-500">
              <XIcon size={30} className='p-1 rounded-full bg-zinc-100'/>
            </button>
            <h2 className="mb-3 text-lg font-normal">Search for Products</h2>
            <input
              type="text"
              value={query}
            ...
            <div className="mt-4 overflow-y-auto max-h-[60vh] pr-2">
              ...
              {!isLoading && results && results.length > 0 && (
                <div className="grid grid-cols-3 gap-2">
                  {results.map((item) => (
                    <div key={item.id} onClick={() => closeAndNavigate(`/products/${item.id}`)} className="flex items-center gap-4 p-2 transition-colors rounded cursor-pointer bg-gray-50 hover:bg-gray-100">
                      <img src={item.image} alt={item.name} className="object-cover w-12 h-12 rounded-md" />
                      <div>
                        <p className="font-medium">{item.name.slice(0, 10)}...</p>
              ...
              {!isLoading && debouncedQuery && results?.length === 0 && (
                <div className="flex flex-col items-center justify-center h-[30vh]">
                  ...
                  <p className="mt-4 text-gray-500">No matching products found for "{debouncedQuery}"</p>
                </div>
              )}
            </div>
          </div>
...
export const MobileBottomNav = () => {
  ...
  return (
    <div className="sticky bottom-0 left-0 right-0 z-[100] block md:hidden bg-green-100 border-t border-gray-200">
      <main className="flex items-center justify-around px-4 py-3">
        {navItems.map((item) => {
